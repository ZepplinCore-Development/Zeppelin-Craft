...
-- Zeppelin fork (F-208): every edit is attributed to a feature/issue id before it
-- is allowed to happen, so `zep world edits emit` can put it in a tagged SQL file.
frame.taskLabel = MakeLabel(frame.leftPanel, "Task (F/I-xxx)", "GameFontNormalSmall")
frame.taskLabel:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 12, -58)

frame.taskBox = MakeEditBox(frame.leftPanel, "", 82)
frame.taskBox:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 18, -74)
frame.taskBox:SetScript("OnEnterPressed", function(self)
    SendGE("TASK:" .. (self:GetText() or ""))
    self:ClearFocus()
end)

frame.taskSetButton = MakeButton(frame.leftPanel, "Set", 30, 22, function()
    SendGE("TASK:" .. (frame.taskBox:GetText() or ""))
end, "Attach every edit in this session to this F-xxx / I-xxx.\nRequired before anything may be moved or saved.")
frame.taskSetButton:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 104, -73)

frame.taskState = MakeLabel(frame.leftPanel, "No task set", "GameFontDisableSmall")
frame.taskState:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 12, -98)
frame.taskState:SetWidth(121)
...
frame.searchButton:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 15, -126)
...
frame.deleteButton:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 15, -164)
...
frame.undoButton:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 15, -202)
...
frame.resetButton:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 15, -240)
...
frame.faceButton:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 15, -278)
...
frame.saveButton:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 15, -316)
...
frame.restoreButton:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 15, -354)

frame.undoStatusText = MakeLabel(frame.leftPanel, "At original position", "GameFontNormalSmall")
frame.undoStatusText:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 12, -425)
...
frame.restoreText:SetPoint("TOPLEFT", frame.leftPanel, "TOPLEFT", 12, -450)
...
-- Zeppelin fork (F-208 / I-359): rebuild the rotation quaternion from the row's
-- own `orientation`. A spawn shipped with a (0,0,0,1) identity quaternion renders
-- at 0 rad no matter what `orientation` says, because that IS a valid unit
-- quaternion and the core never second-guesses it.
frame.fixRotButton = MakeButton(frame.controlsPanel, "Fix Rotation (use orientation)", 240, 20, function()
    SendGE("FIX_ROT")
end, "The selected object's rotation quaternion disagrees with its orientation column.\nRebuild the quaternion from orientation so it renders at the facing the data claims.")
frame.fixRotButton:SetPoint("TOPLEFT", frame.controlsPanel, "TOPLEFT", 180, -104)
frame.fixRotButton:Hide()
...
controlledButtons = {
    frame.taskSetButton,   -- Zeppelin fork (F-208)
    frame.fixRotButton,    -- Zeppelin fork (F-208 / I-359)
    frame.scanButton,
    frame.searchButton,
    frame.deleteButton,
    frame.resetButton,
...
eventFrame:SetScript("OnEvent", function(_, event, ...)
    ...
    if event == "CHAT_MSG_ADDON" then
        ...
        -- Zeppelin fork (F-208): task attribution + rotation mismatch reporting.
        if opcode == "TASK" then
            local sub = parts[2]
            if sub == "SET" then
                state.taskId = parts[3]
                frame.taskBox:SetText(state.taskId or "")
                SetText(frame.taskState, "Editing as " .. (state.taskId or "?"))
                frame.taskState:SetFontObject(GameFontNormalSmall)
                SetText(frame.status, "Status: task " .. (state.taskId or "?"))
            elseif sub == "CLEARED" then
                state.taskId = nil
                SetText(frame.taskState, state.taskRequired
                    and "No task set - editing is locked" or "No task set")
                frame.taskState:SetFontObject(GameFontDisableSmall)
            elseif sub == "REQUIRED" then
                state.taskRequired = (parts[3] == "1")
                if not state.taskId then
                    SetText(frame.taskState, state.taskRequired
                        and "No task set - editing is locked" or "No task set")
                end
            end
        ...
        if opcode == "TASK_FAIL" then
            SetText(frame.status, "Status: bad task id - use F-123 or I-456")
            SetText(frame.taskState, "Invalid task id")
            frame.taskState:SetFontObject(GameFontRedSmall)
            return
        end

        if opcode == "WARN" and parts[2] == "ROTATION_MISMATCH" then
            state.rotationMismatch = { rendered = parts[3], orientation = parts[4] }
            frame.fixRotButton:Show()
            SetText(frame.status, "Status: renders at " .. (parts[3] or "?")
                .. " rad, orientation says " .. (parts[4] or "?"))
            return
        end

        if opcode == "FIX_ROT_OK" then
            state.rotationMismatch = nil
            frame.fixRotButton:Hide()
            SetText(frame.status, "Status: rotation rebuilt from orientation - Save to keep it")
            return
        end

        if opcode == "FIX_ROT_FAIL" then
            frame.fixRotButton:Hide()
            SetText(frame.status, "Status: rotation already matches orientation")
            return
        end

        -- Any handler refusing for want of a task id reports it the same way.
        if parts[2] == "NO_TASK_ID_SET" then
            SetText(frame.status, "Status: set a task id (F/I-xxx) before editing")
            SetText(frame.taskState, "No task set - editing is locked")
            frame.taskState:SetFontObject(GameFontRedSmall)
            return
        end

        if opcode == "SELECTED" then
            -- a fresh selection has not been reported as mismatched yet
            state.rotationMismatch = nil
            frame.fixRotButton:Hide()
        end
