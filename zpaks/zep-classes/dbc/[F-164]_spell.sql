...
-- ----------------------------------------------------------------------------
-- Retired (deleted, unused) — Earthwarden passives removed entirely, not migrated:
--   Shield Mastery  900133-900137 (old block-chance/value passive, superseded by the F-192 block rework)
--   Lightning Ward  900157-900159 (never slotted into any talent / skill line)
-- Defensive passives that WERE kept moved to [F-164M]_spell.sql.
-- Explicit DELETE so `dbc db apply --changed` clears live rows without a full rebuild.
-- ----------------------------------------------------------------------------
DELETE FROM `spell` WHERE `id` IN (900133, 900134, 900135, 900136, 900137, 900157, 900158, 900159);
