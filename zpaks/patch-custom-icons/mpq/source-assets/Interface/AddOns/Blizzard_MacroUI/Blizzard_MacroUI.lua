...
-- Zeppelin F-181: the macro icon picker is fed from the SpellIcon manifest
-- (ZeppelinMacroIcons, loaded first via the .toc) instead of the client's
-- baked-in GetMacroIconInfo list, which can't be extended with custom icons.
-- Normalize a texture path for case/separator-insensitive matching.
function ZeppelinMacroIcon_Normalize(path)
...
function MacroPopupFrame_Update(self)
	...
	local numMacroIcons = #ZeppelinMacroIcons;		-- Zeppelin F-181: SpellIcon manifest
	...
	elseif ( self.mode == "edit" ) then
		...
		MacroPopupFrame.selectedIcon = nil;		-- highlight the edited macro's icon by texture, not a stale index
	end

	-- Icon list (Zeppelin F-181: source is ZeppelinMacroIcons, not GetMacroIconInfo)
	local selectedKey = ZeppelinMacroIcon_Normalize(MacroPopupFrame.selectedIconTexture);
	local texture;
	for i=1, NUM_MACRO_ICONS_SHOWN do
		...
		texture = ZeppelinMacroIcons[index];
		...
		elseif ( selectedKey and (selectedKey == ZeppelinMacroIcon_Normalize(texture)) ) then
			macroPopupButton:SetChecked(1);
		else
...
function MacroPopupButton_SelectTexture(selectedIcon)
	...
	-- Zeppelin F-181: track the texture path so the macro can be saved by path
	MacroPopupFrame.selectedIconTexture = ZeppelinMacroIcons[selectedIcon];
	MacroFrameSelectedMacroButtonIcon:SetTexture(MacroPopupFrame.selectedIconTexture);
	MacroPopupOkayButton_Update();
...
function MacroPopupOkayButton_OnClick(self, button)
	...
	elseif ( MacroPopupFrame.mode == "edit" ) then
		index = EditMacro(MacroFrame.selectedMacro, MacroPopupEditBox:GetText(), icon);
	end
	MacroPopupFrame:Hide();
