...
with open(ZPAK + "/sql/zz_[F-011]_smartai.sql", "w") as f:
    ...
    f.write("-- %d script blocks, %d rows total. %d Cata-only rows skipped (event/action out of 3.3.5a range).\n\n" % (
        len(rows_by_key), sum(len(v) for v in rows_by_key.values()), skipped))
    ...
    for (st, e), rws in sorted(rows_by_key.items()):
        ...
        f.write(",\n".join(vals) + ";\n\n")
print("script blocks: %d | rows: %d | skipped(Cata-only): %d | creatures: %d GOs: %d" % (
    len(rows_by_key), sum(len(v) for v in rows_by_key.values()), skipped, len(cre), len(go)))
