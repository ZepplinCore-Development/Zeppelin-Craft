...
MAPPINGS_FILE = str(Path(__file__).parent / 'data' / 'section_mappings.json')
...
def _load_loot_config() -> dict:
    ...
    try:
        ...
        logger.warning(f"Could not load loot config from {MAPPINGS_FILE}: {e}")
...
class LootDatabase:
    ...
    def __init__(self, host=None, port=None, user=None, password=None, database=None):
        ...
        Args:
            host: Database host address (default: DB_HOST or 192.168.0.55)
            port: Database port (default: DB_PORT or 3306)
            user: Database username (default: DB_USER or acore)
            password: Database password (default: DB_PASS or acore)
            database: Database name (default: acore_world)
        """
    ...
    def get_creature_id(self, boss_name: str) -> Optional[int]:
        ...
        try:
            ...
            if result:
                ...
                logger.warning(f"Boss '{boss_name}' not found in creature_template")
                return None

        except pymysql.Error as err:
            logger.error(f"Query error in get_creature_id: {err}")
            return None
    ...
    def get_boss_loot(self, creature_id: int, include_references: bool = True) -> List[Dict]:
        ...
        try:
            cursor = self.connection.cursor(pymysql.cursors.DictCursor)
            cursor.execute(direct_query, {'creature_id': creature_id})
            results = list(cursor.fetchall())
            cursor.close()

        except pymysql.Error as err:
            logger.error(f"Direct loot query error: {err}")
            return []
    ...
    def _resolve_reference_loot(self, creature_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Reference query error: {err}")
            return []
    ...
    def _get_reference_items(self, reference_id: int, base_group_offset: int = None,
        ...
        try:
            ...
            logger.error(f"Reference items query error: {err}")
            return []
    ...
    def get_all_bosses_loot(self, creature_ids: List[int]) -> Dict[int, List[Dict]]:
        ...
        try:
            ...
            logger.error(f"Query error in get_all_bosses_loot: {err}")
            return {}
    ...
    def get_boss_name(self, creature_id: int) -> Optional[str]:
        ...
        try:
            ...
            if result:
                ...
                logger.warning(f"Creature ID {creature_id} not found")
                return None

        except pymysql.Error as err:
            logger.error(f"Query error in get_boss_name: {err}")
            return None
    ...
    def verify_item_exists(self, item_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Query error in verify_item_exists: {err}")
            return False
    ...
    def get_gameobject_name(self, gameobject_entry: int) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Query error in get_gameobject_name: {err}")
            return None
    ...
    def get_gameobject_loot(self, gameobject_entry: int) -> List[Dict]:
        ...
        try:
            ...
            if not result or not result['loot_id']:
                logger.warning(f"No loot template found for gameobject {gameobject_entry}")
                return []

            loot_template_id = result['loot_id']

        except pymysql.Error as err:
            logger.error(f"Gameobject query error: {err}")
            return []
        ...
        try:
            ...
            logger.error(f"Gameobject loot query error: {err}")
            return []
    ...
    def _resolve_gameobject_reference_loot(self, loot_template_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Gameobject reference query error: {err}")
            return []
