...
from .mappings import get_lua_item_line, get_boss_header_line
...
class LuaGenerator:
    ...
    def generate_single_boss_section(self, loot_items: List[Dict], boss_name: str = None) -> str:
        ...
        if dropped_pools:
            ...
            logger.warning(f"OVERFLOW: {self.display_name} - {len(dropped_pools)} pool(s) dropped ({total_dropped} items) to fit {self.MAX_ITEMS}-slot limit")
            for pool in dropped_pools:
                sample = ', '.join(pool['items'])
                if pool['item_count'] > 3:
                    sample += f", ... (+{pool['item_count'] - 3} more)"
                logger.warning(f"  - Pool {pool['group_id']}: {pool['item_count']} items ({sample})")
