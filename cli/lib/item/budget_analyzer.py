...
- ItemLevel BETWEEN 60 AND 270 (vanilla through ICC 25H)
"""
...
WHERE it.Quality = 4 AND it.stat_type1 > 0
  ...
  AND it.class IN (2, 4)
  AND it.ItemLevel BETWEEN 60 AND 270
...
# (cloth/leather/mail/plate); weapons split by weapon type (axe/mace/sword/etc.).
SUBCLASS_SPLIT_TYPES = {
    # Armor slots
    1,  3,  5,  6,  7,  8,  9,  10, 20,
    # Weapon slots
    13, 15, 17, 21, 22, 23, 25, 26, 28,
}
...
def _fetch_reference_items() -> Tuple[
    ...
    Returns:
        by_slot_subclass: keyed by (InventoryType, subclass) for ARMOR slots
        by_slot: keyed by InventoryType only (everything, used as fallback)
    """
    ...
    try:
        ...
        conn.close()
    return by_slot_subclass, by_slot
...
def analyze(verbose: bool = True) -> Dict:
    ...
    Result shape:
        {
          "formulas": {                  # key: "<iv>" for non-armor / fallback
            "<iv>": {"a": ..., "b": ..., "c": ..., "r2": ..., "n": ...},
          },
          "formulas_by_subclass": {      # key: "<iv>:<subclass>" for armor splits
            "5:4": {...},                # plate chest
          },
          "quality_ratio": {"3": 0.85, "4": 1.0},
          "min_samples": 20,
          "armor_split_slots": [1, 3, 5, ...]
        }
    """
    by_slot_subclass, by_slot = _fetch_reference_items()
    if verbose:
        total = sum(len(v) for v in by_slot.values())
        print(f"Loaded {total} stock epic items across {len(by_slot)} inventory types")
        print()
        print("Per-slot fits (used when no subclass-specific fit available):")
    ...
    if verbose:
        print()
        print("Per-(slot, subclass) fits (armor only):")
