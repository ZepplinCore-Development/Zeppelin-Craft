...
def _preprocess_block_data(zpak: Dict[str, Any]) -> bool:
    ...
    out = (zpak_path / 'mpq' / 'parsed-assets' / 'INTERFACE' / 'ADDONS' /
           'ELVUI_ENHANCED' / 'ZEPBLOCKDATA.LUA')
    ...
    lines += ["\t},", "}"]
    out.write_text('\n'.join(lines) + '\n')
    print(f"    ZepBlockData.lua generated (percentMode={percent_mode}, {len(per_pct)} levels)")
    return True
