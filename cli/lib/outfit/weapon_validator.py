...
def validate_display_info(dbc_cursor, acore_cursor):
    ...
    click.echo("=" * 80)
    click.echo("VALIDATING DISPLAY INFO")
    click.echo("=" * 80)
    click.echo()
    ...
    click.echo(f"Found {len(mismatches)} display_item_ mismatches")
    if mismatches:
        click.echo()
        click.echo("Mismatches found:")
        for m in mismatches:
            click.echo(f"  {m['race']} {m['class']} ({m['gender']}) slot {m['slot']}: "
...
def validate_starter_weapons(cursor):
    ...
    """
    click.echo("=" * 80)
    click.echo("VALIDATING STARTER WEAPON ITEMS")
    click.echo("=" * 80)
    click.echo()
    ...
    for skill_id, candidate_items in STARTER_WEAPON_CANDIDATES.items():
        ...
        click.echo(f"{skill_name} (skill {skill_id}):")
        ...
        if not item:
            click.echo(f"  x {item_id}: Item not found in database!")
            continue
        ...
        if item_class == 2:  # Weapon
            ...
            if subclass == expected_subclass:
                click.echo(f"  + {entry} ({name}): Valid starter weapon")
                validated_weapons[skill_id] = entry
            else:
                click.echo(f"  x {entry} ({name}): Wrong subclass")
        else:
            click.echo(f"  x {entry} ({name}): Not a weapon")
...
def find_duplicate_weapons(dbc_cursor, acore_cursor):
    ...
    click.echo(f"Found {len(duplicate_cleanups)} duplicate weapons/ammo to clean up")
    click.echo()

    return duplicate_cleanups


def validate_weapon_coverage(weapon_skills, source_cursor, original_cursor, acore_cursor,
                             validated_weapons, add_dk_melee, ranged_classes, ranged_combos):
    """
    ...
    Args:
        ...
        add_dk_melee: Whether to add melee weapons to Death Knights
        ranged_classes: Set of class IDs that get ranged weapons
        ranged_combos: Set of (race_id, class_id) tuples for ranged weapons
    ...
    for outfit in outfits:
        ...
        skip_melee_addition = (class_id == 6 and not add_dk_melee)
        ...
        # BUT only for configured classes
        allowed_ranged = (class_id in ranged_classes or
                         (race_id, class_id) in ranged_combos)
    ...
    click.echo(f"Skipped {skipped_invalid} invalid race/class combinations")
    click.echo(f"Found {len(mismatches)} weapon mismatches")
    click.echo(f"Found {len(weapons_only)} weapon additions needed")
    click.echo(f"Found {len(ammo_only)} ammo additions needed")
    click.echo()
