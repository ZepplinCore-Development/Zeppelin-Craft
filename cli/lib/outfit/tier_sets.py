...
# Markers for tier set section in zpak SQL file
TIER_SECTION_START = "-- [TIER SET DISPLAY OVERRIDES - START]"
TIER_SECTION_END = "-- [TIER SET DISPLAY OVERRIDES - END]"
...
def write_tier_sql_to_zpak(tier_key, tier_display_name, sql_statements, zpak_dbc_dir):
    ...
    Args:
        tier_key: Tier set key (e.g. 'tier2')
        tier_display_name: Human-readable tier name
        sql_statements: List of SQL statements from apply_tier_set()
    ...
    lines = []
    lines.append("")
    lines.append(TIER_SECTION_START)
    lines.append("-- " + "=" * 76)
    lines.append("-- TIER SET DISPLAY OVERRIDES")
    lines.append(f"-- Applied tier: {tier_display_name}")
    lines.append(f"-- AUTO-GENERATED by: zep outfit tier apply {tier_key}")
    lines.append("-- DO NOT EDIT MANUALLY - Regenerated on each tier apply")
    lines.append("-- " + "=" * 76)
    lines.append("")

    for stmt in sql_statements:
        # Strip dbc. prefix for zpak format
        clean = stmt.replace('`dbc`.`charstartoutfit`', '`charstartoutfit`')

        # Split inline comment from SQL
        # Format: "UPDATE ... WHERE `id` = 123; -- comment"
        if '; -- ' in clean:
            sql_part, comment_part = clean.split('; -- ', 1)
            lines.append(f"-- {comment_part}")
            lines.append(f"{sql_part};")
        else:
            lines.append(clean)
        lines.append("")

    lines.append(TIER_SECTION_END)
    ...
    # Remove any existing tier section
    if TIER_SECTION_START in existing:
        start_idx = existing.index(TIER_SECTION_START)
        end_idx = existing.find(TIER_SECTION_END)
        if end_idx >= 0:
            after = existing[end_idx + len(TIER_SECTION_END):]
            existing = existing[:start_idx].rstrip() + after.lstrip('\n')
        else:
            existing = existing[:start_idx].rstrip()

    # Write back with tier section appended
...
def remove_tier_sql_from_zpak(zpak_dbc_dir):
    ...
    if TIER_SECTION_START not in content:
        return False

    start_idx = content.index(TIER_SECTION_START)
    end_idx = content.find(TIER_SECTION_END)
    if end_idx >= 0:
        after = content[end_idx + len(TIER_SECTION_END):].lstrip('\n')
