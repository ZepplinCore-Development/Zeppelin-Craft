...
# =============================================================================
# TRS Commands (F-201) - md5translate.trs managed as a pseudo-DBC table
# =============================================================================

@dbc.group('trs')
@click.pass_context
def trs(ctx):
    """md5translate.trs (minimap texture table) as a DBC-style table.

    The client's textures/minimap/md5translate.trs is managed like a DBC:
    original_dbc.md5translate holds the stock layer, zpak dbc/ SQL files
    layer modifications, and the PATCH-Z build serializes the live table
    into a complete .trs via the 'trs-generate' preprocessor.
    """
    pass


def _trs_db_name(config, database: str) -> str:
    return {
        'live': config.live,
        'original': config.original,
        'expected': config.expected,
...
@click.pass_context
def trs_init(ctx):
    """Create the md5translate table in original/live/expected databases."""
    from lib.trs_utils import CREATE_TABLE_SQL, TABLE

    config = get_dbc_config(ctx)
    with DBCConnection(config) as db_conn:
        for database in (config.original, config.live, config.expected):
            conn = db_conn.get_connection(database)
            cur = conn.cursor()
            cur.execute(CREATE_TABLE_SQL)
            conn.commit()
            cur.close()
            click.echo(f"  {database}.{TABLE}: OK")
    click.echo(click.style("TRS table initialized", fg='green'))


@trs.command('import-original')
@click.argument('trs_file', type=click.Path(exists=True))
@click.option('--force', '-f', is_flag=True, help='Skip confirmation prompt')
@click.pass_context
def trs_import_original(ctx, trs_file: str, force: bool):
    """Import a .trs file as the stock layer (original_dbc.md5translate).

    Wipes and reloads the stock layer. Use when a pristine stock client
    trs becomes available, then regenerate whole-file zpak layers with
    'zep dbc trs diff'. Duplicate src lines collapse last-wins (client
    semantics).
    """
    from pathlib import Path as _Path
    from lib.trs_utils import TABLE, parse_trs_file

    config = get_dbc_config(ctx)
    entries = parse_trs_file(_Path(trs_file))
    click.echo(f"Parsed {len(entries)} entries from {trs_file}")

    if not force and not click.confirm(
            f"Replace ALL rows in {config.original}.{TABLE}?"):
        click.echo("Cancelled.")
        return

    with DBCConnection(config) as db_conn:
        conn = db_conn.get_connection(config.original)
        cur = conn.cursor()
        cur.execute(f"DELETE FROM `{TABLE}`")
        cur.executemany(
            f"INSERT INTO `{TABLE}` (`src`, `dst`, `dir`) VALUES (%s, %s, %s)",
            list(entries.values()))
        conn.commit()
        cur.close()
    click.echo(click.style(
        f"Imported {len(entries)} rows into {config.original}.{TABLE}", fg='green'))


@trs.command('diff')
@click.argument('trs_file', type=click.Path(exists=True))
@click.option('--output', '-o', 'output', type=click.Path(), required=True,
              help='Output SQL file (zpak dbc/ path)')
@click.option('--task', '-t', 'task_id', default='F-201',
              help='Tracking ID for the header (default: F-201)')
@click.pass_context
def trs_diff(ctx, trs_file: str, output: str, task_id: str):
    """Diff a whole .trs file against the stock layer, emit zpak SQL.

    For zpaks that ship a complete md5translate.trs (e.g. Open Azeroth):
    computes file-minus-stock and writes an idempotent DELETE+INSERT SQL
    layer. Entries removed relative to stock are emitted as DELETEs.
    """
    from pathlib import Path as _Path
    from lib.trs_utils import TABLE, parse_trs_file, diff_rows, emit_sql, sql_str

    config = get_dbc_config(ctx)
    file_entries = parse_trs_file(_Path(trs_file))
    ...
    with DBCConnection(config) as db_conn:
        ...
        for src, dst, d in cur.fetchall():
            base_entries[src.lower()] = (src, dst, d)
        cur.close()

    changed, removed = diff_rows(file_entries, base_entries)
    header = [
        f"-- [{task_id}] md5translate layer generated by 'zep dbc trs diff' - DO NOT EDIT",
        f"-- Source: {_Path(trs_file).name}",
        f"-- {len(changed)} changed/new entries, {len(removed)} removed vs stock layer",
        f"-- Regenerate after upstream updates: zep dbc trs diff <new.trs> -o <this file>",
        "",
    ]
    sql = emit_sql(changed, header)
    if removed:
        keys = ', '.join(sql_str(s) for s in sorted(removed, key=str.lower))
        sql += f"\n-- Entries removed relative to the stock layer\n"
        sql += f"DELETE FROM `{TABLE}` WHERE `src` IN ({keys});\n"
    _Path(output).write_text(sql)
    click.echo(click.style(
        f"Wrote {output}: {len(changed)} changed/new, {len(removed)} removed",
        fg='green'))


@trs.command('export')
@click.option('--output', '-o', 'output', type=click.Path(), required=True,
              help='Output .trs file path')
@click.option('--database', '-d', 'database',
              type=click.Choice(['live', 'original', 'expected']),
              default='live', help='Source database (default: live)')
@click.pass_context
def trs_export(ctx, output: str, database: str):
    """Serialize the md5translate table into a complete .trs file."""
    from pathlib import Path as _Path
    from lib.trs_utils import export_trs_from_db

    config = get_dbc_config(ctx)
    count = export_trs_from_db(config, _trs_db_name(config, database),
                               _Path(output))
    click.echo(click.style(f"Wrote {output}: {count} entries", fg='green'))
